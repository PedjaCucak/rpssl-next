...
export async function randomChoiceId(signal?: AbortSignal): Promise<1|2|3|4|5> {
  try {
    const ctrl = new AbortController();
    const t = setTimeout(() => ctrl.abort(), 800);
    const res = await fetch('https://codechallenge.boohma.com/random', { signal: ctrl.signal });
    clearTimeout(t);
